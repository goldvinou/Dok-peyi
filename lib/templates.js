...
function _renderCV(data, personal) {
  ...
  const fullName = _fullName(personal);
...
/** Return "Prénom Nom" (trims extra whitespace; handles missing parts). */
function _fullName(personal) {
  return [personal.prenom, personal.nom].filter(Boolean).join(' ');
}

/**
 * Render the contact chips (email + phone) shared by all header sections.
 * Returns an HTML fragment (does not include a wrapping element).
 */
function _renderContacts(personal) {
  return [
