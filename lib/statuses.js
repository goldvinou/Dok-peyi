 ...
 *
 * Standard flow (no review):
 *   submitted → processing → generated → pending_payment → paid → delivered
 *
 * Pre-payment review (sejour / mandatory):
 *   generated → needs_review → pending_payment → paid → delivered
 *
 * Post-payment review (dossier sensitive):
 *   generated → pending_payment → paid → needs_review → delivered
 *
 * Any status → failed (via the fail action).
 * Final document unlocked at: paid, needs_review, delivered.
 */
...
const DOK_STATUS_META = Object.freeze({
  ...
  processing:      { label: 'En traitement',     cssClass: 'processing',      terminal: false },
 ...
 *
 * NOTE: This mirrors VALID_TRANSITIONS in lib/pipeline.js, which is the
 * authoritative server-side copy. Keep both in sync when adding statuses.
 */
const DOK_STATUS_TRANSITIONS = Object.freeze({
  ...
  // pending_payment: admin approved a pre-payment review; client now pays
  // delivered:       post-payment review done (payment already recorded)
