...
export const handlers = {
   ...
   *                                                     stays at generated
   *
   * Calls the Anthropic API, stores the clean document, then runs the
   * review engine (lib/review.js) to decide final routing:
   *   - reviewRequired=false → order.statut = 'generated' (proceed to payment)
   *   - reviewRequired=true  → order.statut = 'needs_review' (admin handles it)
  ...
  async generate(order, { prompt, apiKey, systemPrompt = '' }) {
    ...
    /* Step D — store clean final document (preview derived on demand via addWatermark) */
    const withDoc = {
      ...toGenerated.order,
      _documents: { final: content }
    };

    /* Step E — evaluate review rules (lib/review.js) */
    ...
    if (review.reviewRequired) {
      /* generated → needs_review: admin must handle before delivery */
      const toReview = applyTransition(withReview, 'needs_review');
      if (!toReview.ok) {
        // Transition guard failed unexpectedly — return generated with flags anyway
        return { ok: true, order: withReview };
      }
      return { ok: true, order: toReview.order };
    }

    /* No review needed — order stays at generated, ready for payment */
    return { ok: true, order: withReview };
  },
