...
export function buildPrompt(order) {
  ...
  switch (service) {
    ...
    case 'sejour':
      return _sejourPrompt(nom, email, tel, details, choice);
...
/** Titre de séjour — returns HTML */
function _sejourPrompt(nom, email, tel, d, choice) {
  return {
    format: 'html',
    prompt: `Tu es spécialiste des titres de séjour en Guyane et en France. Génère un guide personnalisé en HTML (CSS inline, A4).
Client : ${nom} | Email : ${email} | Tél : ${tel} | Nationalité : ${d.nationalite || ''}
Type : ${choice} | Situation : ${d.situation || ''} | Documents disponibles : ${d.documents || ''}
Contenu : 1) résumé 2) démarches étape par étape 3) checklist ☐ 4) organismes Guyane 5) délais et points de vigilance.
Bandeau d'avertissement visible : "Ce document est une aide. Il ne remplace pas un conseil juridique."
En-tête fond rouge #b91c1c, accents #ef4444.
${HTML_INSTRUCTION}`
  };
}
