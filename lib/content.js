...
function _dossierPrompt(nom, email, tel, d, choice) {
  if (choice === 'logement') return _dossierLogementPrompt(nom, d);
  if (choice === 'aide')     return _dossierAidePrompt(nom, d);
  if (choice === 'autre')    return _dossierAutrePrompt(nom, d);
  return _dossierCafPrompt(nom, d, choice);
}

function _dossierCafPrompt(nom, d, choice) {
  return {
    format: 'html',
    prompt: `Expert démarches CAF Guyane. Génère un guide HTML A4 CSS-inline.
1. RÉSUMÉ : prestation demandée + montant estimatif si calculable
2. DOCUMENTS OBLIGATOIRES : checklist ☐ exhaustive avec validité (CNI 10 ans, justif domicile -3 mois, etc.)
3. ÉTAPES NUMÉROTÉES : procédure exacte CAF 973
4. DÉLAIS RÉELS : traitement CAF Guyane 4-8 semaines (délais DOM supérieurs métropole)
5. PIÈGES FRÉQUENTS : 5 erreurs qui bloquent le dossier CAF Guyane
6. CONTACTS :
   - CAF 973 : 20 rue Christophe Colomb, Cayenne — 0810 25 97 10 (lun-ven 9h-16h)
   - Espace numérique : caf.fr rubrique 'Ma CAF'
   - Permanences associatives : AIDES, CIMADE Guyane
Client : ${nom} | Type CAF : ${choice} | Situation : ${d.description || ''} | Documents disponibles : ${d.documents || ''}
RÈGLE : mentionner systématiquement le délai DOM-TOM majoré et le droit au recours si refus.`
  };
}

function _dossierLogementPrompt(nom, d) {
  return {
    format: 'html',
    prompt: `Expert logement social Guyane. Génère un guide HTML A4 CSS-inline.
1. NUMÉRO UNIQUE : explication SNE (Système National d'Enregistrement) + comment obtenir le numéro
2. DOCUMENTS OBLIGATOIRES : checklist ☐ avec validité
3. BAILLEURS SOCIAUX GUYANE :
   - Simko : principal bailleur — 18 av du Général de Gaulle, Cayenne — 05 94 25 08 08
   - SIGUY : 18 rue Léopold Héder, Cayenne — 05 94 30 07 30
   - CDC Habitat Guyane : 05 94 29 80 00
4. DEAL GUYANE : 11 rue de la Madeleine, Cayenne — dossiers prioritaires DALO
5. DÉLAIS RÉELS : 2-7 ans en Guyane (tension locative extrême)
6. RECOURS DALO : conditions + procédure si délai anormal
Client : ${nom} | Situation : ${d.description || ''} | Documents disponibles : ${d.documents || ''}
RÈGLE : toujours mentionner le recours DALO comme option si délai dépasse seuil légal.`
  };
}

function _dossierAidePrompt(nom, d) {
  return {
    format: 'html',
    prompt: `Expert aides sociales Guyane. Génère un guide HTML A4 CSS-inline.
1. AIDES IDENTIFIÉES : liste des aides auxquelles ce profil est potentiellement éligible
2. POUR CHAQUE AIDE : montant, conditions, organisme, documents, délai
3. ORGANISMES GUYANE :
   - CCAS Cayenne : Hôtel de Ville, pl. Général de Gaulle — 05 94 29 55 00
   - DJSCS Guyane : 6 rue Christophe Colomb — 05 94 29 83 00
   - MSA Guyane (agriculteurs) : 05 94 29 09 09
   - CGSS Guyane (sécu sociale) : route de Baduel, Cayenne — 05 94 29 54 00
4. AIDES D'URGENCE : FSL, aide alimentaire CCAS, épiceries solidaires Cayenne
5. DÉMARCHE RECOMMANDÉE : ordre dans lequel faire les demandes
Client : ${nom} | Situation : ${d.description || ''} | Documents disponibles : ${d.documents || ''}
RÈGLE : toujours inclure au moins une aide d'urgence accessible rapidement.`
  };
}

function _dossierAutrePrompt(nom, d) {
  return {
    format: 'html',
    prompt: `Expert administratif Guyane. Génère un guide HTML A4 CSS-inline pour cette démarche.
1. CONTEXTE : résumé de la situation et de l'objectif
2. DÉMARCHE : étapes numérotées claires et actionnables
3. DOCUMENTS : checklist ☐ avec validité
4. ORGANISME COMPÉTENT : adresse réelle Guyane si applicable
5. DÉLAI ESTIMÉ : réaliste pour la Guyane
6. RECOURS : option si refus ou délai dépassé
Client : ${nom} | Type : ${d.type || 'autre'} | Situation : ${d.description || ''} | Documents : ${d.documents || ''}
RÈGLE : si organisme non identifiable, orienter vers CCAS Cayenne comme point d'entrée universel.`
  };
}
